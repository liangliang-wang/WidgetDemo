...
//*** 身份证验证 结束 ***
//IP验证
var IPAddress = /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/;
$.validator.addMethod("IP", function (value, element) {
    return this.optional(element) || IPAddress.test(value.replace(/(^\s*)|(\s*$)/g, ""));
}, "请正确输入IP地址");
//正则表达式
...
function CharCodeLength(s) {
    var n = 0;
    for (var i = 0; i < s.length; i++) {//charCodeAt()可以返回指定位置的unicode编码,这个返回值是0-65535之间的整数  
...
$.validator.addMethod("XmlString", function (value, element) {
...
//日期大于与小于
function DateThan(date1, date2) {
    var date1 = date1.replace(/-/g, "/");
    var date2 = $(date2).val().replace(/-/g, "/");
    if (date1.split("/").length == 2) date1 = date1 + "/1";
    if (date2.split("/").length == 2) date2 = date2 + "/1";
    return new Date(date1) -  new Date(date2);
}
$.validator.addMethod("DateThanBig", function (value, element, param) {
    if (value.trim().length > 0) return DateThan(value, param) > 0;
    return true;
}, "需要大于开始时间");
$.validator.addMethod("DateThanSmall", function (value, element, param) {
    if (value.trim().length > 0 && $(param).val().trim().length > 0) return DateThan(value, param) < 0;
    return true;
}, "需要小于结束时间");
/******************************* 增加 验证 结束 *******************************/
